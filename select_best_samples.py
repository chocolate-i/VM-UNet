...
import random
import logging
import logging.handlers# 添加一个简化版的保存函数
from tqdm import tqdm
def save_sample_images(img, msk, pred, index, save_dir, metric_name, metric_value):
    """保存样本图像而不依赖SimpleITK"""
    ...
    # 保存原始numpy数据
    np.save(f"{save_dir}/best_{index}_{metric_name}_{metric_value:.4f}_img.npy", img)
    np.save(f"{save_dir}/best_{index}_{metric_name}_{metric_value:.4f}_mask.npy", msk)
    np.save(f"{save_dir}/best_{index}_{metric_name}_{metric_value:.4f}_pred.npy", pred)
    ...
    plt.title(f'Prediction ({metric_name}={metric_value:.4f})')
    plt.axis('off')
    
    plt.savefig(f"{save_dir}/best_{index}_{metric_name}_{metric_value:.4f}.png")
    plt.close()
...
def select_best_samples(model, data_loader, config, save_dir, ratio=0.05, metric_name='dice'):
    """选择分割效果最好的样本并保存"""
    model.eval()
    ...
    with torch.no_grad():
        for i, data in enumerate(tqdm(data_loader)):
            ...
            # 计算Dice系数
            threshold = config.threshold
            binary_pred = np.where(out_np >= threshold, 1, 0)
            binary_gt = np.where(msk_np >= 0.5, 1, 0)

            # 计算评估指标
            if binary_pred.sum() > 0 and binary_gt.sum() > 0:
                dice = metric.binary.dc(binary_pred, binary_gt)
                iou = metric.binary.jc(binary_pred, binary_gt)
            elif binary_pred.sum() > 0 and binary_gt.sum() == 0:
                dice = 0
                iou = 0
            else:
                dice = 0
    ...
    # 根据指定指标排序 - 这里使用 metric_name 而不是 metric
    sorted_results = sorted(results, key=lambda x: x['metrics'][metric_name], reverse=True)
    
    # 选择前ratio%的样本
    num_to_select = max(1, int(len(sorted_results) * ratio))
    selected_samples = sorted_results[:num_to_select]
    
    print(f"选择了{num_to_select}个最佳样本 (比例: {ratio:.2f})")
    ...
    for i, sample in enumerate(selected_samples):
        ...
        msk = sample['mask']
        pred = sample['pred']
        score = sample['metrics'][metric_name]  # 使用 metric_name
        ...
        save_imgs(
            ...
            config.datasets, 
            config.threshold,
            test_data_name=f"best_{i+1}_{metric_name}_{score:.4f}"
        )
        
        print(f"保存样本 {i+1}/{num_to_select}, 索引: {idx}, {metric_name}: {score:.4f}")
